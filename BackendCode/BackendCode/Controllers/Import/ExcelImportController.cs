...
using Achieve.Model;
//using Achieve.Model.Models;
using BackendCode.Tran;
using Achieve.IRepository;
...
namespace BackendCode.Controllers
{
    ...
    {
        private readonly IPaperClassRepository _iPaperClassRepository;
        private readonly IUsersRepository _iUsersRepository;
        private readonly ILikesRepository _iLikesRepository;
        private readonly ICollectionsRepository _iCollectionsRepository;
        private readonly IClicksRepository _iClicksRepository;

        public ExcelImportController(IPaperClassRepository iClazzRepository, ICollectionsRepository iCollectionsRepository, IClicksRepository iClicksRepository, IUsersRepository iStudentsRepository, ILikesRepository iExScoreRepository)
        {
            this._iPaperClassRepository = iClazzRepository;
            this._iUsersRepository = iStudentsRepository;
            this._iLikesRepository = iExScoreRepository;
            this._iClicksRepository = iClicksRepository;
            this._iCollectionsRepository = iCollectionsRepository;
        }
                    ...
                    foreach (var grade in grades)
                    {
                        ...
                        if (gradeId > 0)
                        {
                            ...
                            foreach (var clazz in clazzs)
                            {
                                ...
                                //clazzid = await _iPaperClassRepository.Add(clazz);
        ...
        public async Task<bool> ImportStudentData(string filepath)
        {
            ...
            try
            {
                ...
                var clazzList = await _iPaperClassRepository.Query(d => d.IsDeleted == false);
        ...
        public async Task<bool> ImportExScoreData(string filepath)
        {
            ...
            try
            {
                ...
                var studentsList = await _iUsersRepository.Query(d => d.IsDeleted == false);
                var gradeList = await _iGradeRepository.Query(d => d.IsDeleted == false);
                var clazzList = await _iPaperClassRepository.Query(d => d.IsDeleted == false);
                ...
                try
                {
                    ...
                    for (int i = 0; i < operateCount; i++)
                    {
                        ...
                        //var exScoreSave = await _iLikesRepository.Add(wanttosave);
        ...
        public async Task<bool> ImportBaseSortData(string filepath)
        {
            ...
            try
            {
                ...
                var examList = await _iExamRepository.Query(d => d.IsDeleted == false);
                var exScoresList = await _iLikesRepository.Query(d => d.IsDeleted == false);
        ...
        public async Task<bool> ImportEDetailScoreData(string filepath)
        {
            ...
            var exScoreList = await _iLikesRepository.Query(d => d.IsDeleted == false);
...

            {
                ...
                for (int i = 0; i < operateCount; i++)
                {
                    ...
                    var exScoreSave = await _iLikesRepository.Update(wanttosave);
                }
